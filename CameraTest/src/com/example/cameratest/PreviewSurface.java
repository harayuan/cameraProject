...
public class PreviewSurface extends SurfaceView implements Callback {
	...
	public void setFocus(Rect rect) {
		...
		if (m_parameters.getMaxNumFocusAreas() == 0) {
			return;
		}

		// calculate the focus area
		ArrayList<Camera.Area> focusArea = new ArrayList<Camera.Area>();
		focusArea.add(new Camera.Area(rect, 1000));

		m_parameters.setFocusAreas(focusArea);
		m_camera.setParameters(m_parameters);
		m_camera.startPreview();
		Log.i("camera", "PreviewSurface: has added focusArea: " + (rect.right - rect.left) + ", " + (rect.bottom - rect.top));
		setCameraFocus();
	}
}
